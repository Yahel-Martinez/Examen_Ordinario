const express = require('express');
const router = express.Router();

router.get('/', (req, res) => {
  req.db.all('SELECT * FROM Clientes', [], (err, rows) => {
    res.json(err ? { error: err.message } : rows);
  });
});

router.get('/:id', (req, res) => {
  req.db.get('SELECT * FROM Clientes WHERE id = ?', [req.params.id], (err, row) => {
    res.json(err ? { error: err.message } : row);
  });
});

router.post('/', (req, res) => {
  const { nombre, correo, telefono, direccion } = req.body;
  req.db.run(
    'INSERT INTO Clientes (nombre, correo, telefono, direccion) VALUES (?, ?, ?, ?)',
    [nombre, correo, telefono, direccion],
    function (err) {
      res.json(err ? { error: err.message } : { id: this.lastID });
    }
  );
});

router.put('/:id', (req, res) => {
  const { nombre, correo, telefono, direccion } = req.body;
  req.db.run(
    'UPDATE Clientes SET nombre = ?, correo = ?, telefono = ?, direccion = ? WHERE id = ?',
    [nombre, correo, telefono, direccion, req.params.id],
    function (err) {
      res.json(err ? { error: err.message } : { updated: this.changes });
    }
  );
});

router.delete('/:id', (req, res) => {
  req.db.run('DELETE FROM Clientes WHERE id = ?', [req.params.id], function (err) {
    res.json(err ? { error: err.message } : { deleted: this.changes });
  });
});

module.exports = router;
